    ...
    if key in current_values:
        if key in previous_values:
            ...
            if current_value != previous_value:
                print("[+] CHANGED  - {}: {} changed from {} to {}"
                      .format(current_ip, key, previous_value, current_value))
        else:
            print("[+] NEW      - {}: {} added".format(current_ip, key))
...
def compare_dicts(previous_result, current_result) -> dict:
    ...
    for current_ip, current_values in current_result.items():
        ...
        if current_ip not in previous_result:
            ...
            print("[+] NEW      - {}: IP not found earlier".format(current_ip))
        ...
        for current_port in current_values.get("ports", []):
            ...
            if position == -1:
                ...
                print("[+] NEW      - {}: New port {} open (Name:{}, Tunnel:{})"
                      .format(current_ip, c_port.port_id, c_port.service_name, c_port.service_tunnel))
                continue

            # Check existing values            
            p_port = PortObject(previous_values.get("ports", [])[position])
            if c_port.service_name != p_port.service_name:
                print("[+] CHANGED  - {}: Port {} Service.Name changed from {} to {}"
                      .format(current_ip, c_port.port_id, c_port.service_name, p_port.service_name))
            if c_port.service_tunnel != p_port.service_tunnel:
                print("[+] CHANGED  - {}: Port {} Service.Tunnel changed from {} to {}"
                      .format(current_ip, c_port.port_id, c_port.service_tunnel, p_port.service_tunnel))
    ...
    for previous_ip, previous_values in previous_result.items():
        ...
        if previous_ip not in current_result:
            ...
            print("[-] REMOVED  - {}: IP no longer exist (Hostname: {})".format(previous_ip, previous_hostname))
        ...
        for previous_port in previous_values.get("ports", []):
            ...
            if find_port_id_position(current_values.get("ports", []), port.port_id) == -1:
                ...
                print("[-] REMOVED  - {}: Old port {} not longer open (Name:{}, Tunnel:{})"
                      .format(previous_ip, port.port_id, port.service_name, port.service_tunnel))
                continue
    ...
    if current_result['flags'] != previous_result['flags']:
        ...
        for item in previous_result['flags']:
            if item not in current_result['flags']:
                flag_changes.append({"RESOLVED": item})
                print('[-] RESOLVED - Flag: ' + str(item) + ' has been resolved')
        ...
        for item in current_result['flags']:
            if item not in previous_result['flags']:
                flag_changes.append({"NEW": item})
                print('[+] NEW      - New flag found ' + str(item) + ' has been found')

        if flag_changes:
            changes.update({"flags": flag_changes})
